// mini_co.cpp — minimal libco-like runtime (~150 lines).
// How it maps to libco:
//   libco stCoRoutineEnv_t::pCallStack  -> t_stack (resume chain)
//   libco coctx_swap (x86 asm)          -> mini_co_swap (ARM64/x86_64 asm, no ucontext)
...
//
// Historically this used getcontext/makecontext/swapcontext, which Apple
// deprecated since macOS 10.6 (warnings on every build). We now use a tiny
// hand-rolled context (callee-saved regs + SP only, which is all a
// resume/yield call needs to preserve) so there are no deprecated calls.
// Linux x86_64 without deprecation warnings keeps the portable ucontext path.
#include "mini_co.h"
#include <vector>
#include <cstdlib>
#include <cassert>
...
// Use hand-rolled asm where ucontext is deprecated/broken:
//   - all ARM64 (Apple Silicon + Linux ARM64)
//   - macOS x86_64 (deprecated ucontext there too)
#if defined(__aarch64__) || (defined(__APPLE__) && defined(__x86_64__))
#define MINI_CO_CUSTOM_CTX 1
#else
#define MINI_CO_CUSTOM_CTX 0
#endif

#if MINI_CO_CUSTOM_CTX
...
#else // Linux ARM64 (ELF, no underscore)
__asm__(
".text\n"
".p2align 2\n"
".globl mini_co_swap\n"
...
"mini_co_entry:\n"
"  mov x0, x19\n"
"  bl mini_co_trampoline_c\n"
"  brk #0\n"
);
#endif

#else // __x86_64__ on macOS
struct mini_ctx_t {
    void* rbx; // 0
    void* rbp; // 8
    void* r12; // 16
...
#else // MINI_CO_CUSTOM_CTX == 0: portable ucontext fallback (Linux x86_64)
#define _XOPEN_SOURCE 700
#include <ucontext.h>

struct mini_co_t {
    ucontext_t ctx;     // own context (== libco coctx_t)
    ucontext_t* caller; // context to swap back to on yield
    char* stack;        // own stack (== libco stack_mem)
    mini_co_fn_t fn;
    void* arg;
    bool finished;
...
// libco: __thread gCoEnvPerThread. Here: main ctx + resume chain per thread.
static thread_local ucontext_t t_main;
static thread_local std::vector<mini_co_t*> t_stack;
static thread_local std::vector<mini_co_t*> t_all; // idx passed via makecontext (int-safe)

static void trampoline(int idx) {
    // makecontext() only passes ints portably, so pass index into t_all.
    mini_co_t* co = t_all[idx];
    co->fn(co->arg);
    co->finished = true;
    mini_co_yield(); // return to caller; never resume a finished co
    abort();         // unreachable
...
int mini_co_create(mini_co_t** out, mini_co_fn_t fn, void* arg) {
    ...
    co->ctx.uc_stack.ss_size = kStackSize;
    co->ctx.uc_link = &t_main; // safety if trampoline ever returns
    int idx = (int)t_all.size();
    t_all.push_back(co);
    makecontext(&co->ctx, (void(*)())trampoline, 1, idx);
    *out = co;
    return 0;
}
...
void mini_co_resume(mini_co_t* co) {
    if (co->finished) return;
    ucontext_t* caller = t_stack.empty() ? &t_main : &t_stack.back()->ctx;
    co->caller = caller;
    t_stack.push_back(co);
    swapcontext(caller, &co->ctx);
    // resumed here when co yields or finishes
}
...
void mini_co_yield() {
    ...
    mini_co_t* cur = t_stack.back();
    t_stack.pop_back();
    swapcontext(&cur->ctx, cur->caller);
}
